...
export const SETTING_HINTS: Record<string, string> = {
  ...
  // ── Line Waves ────────────────────────────────────────────────────────────────
  'background.lineWavesEnabled':        'Animated warped line pattern that fills the background.',
  'background.lineWavesBehindLogo':     'When set to "Behind logo", the effect is masked out at the logo position. "In front" lets it cover everything including the logo.',
  'background.lineWavesBlendMode':      'How the line waves layer blends with the layers beneath it.',
  'background.lineWavesSpeed':          'Animation speed of the wave warp. Higher = faster motion.',
  'background.lineWavesInnerLines':     'Number of line bands in the centre region of the screen.',
  'background.lineWavesOuterLines':     'Number of line bands toward the edges of the screen.',
  'background.lineWavesWarpIntensity':  'How strongly the lines bend and distort. 0 = straight, higher = chaotic.',
  'background.lineWavesRotation':       'Global rotation of the whole line wave pattern in degrees.',
  'background.lineWavesEdgeFadeWidth':  'How far from the centre edge-fade kicks in. 0 = no fade, higher = narrower band.',
  'background.lineWavesColorCycleSpeed':'Speed of the colour cycling animation across the wave pattern.',
  'background.lineWavesBrightness':     'Overall brightness of the line waves. Higher = more intense glow.',
  'background.lineWavesColor1':         'First colour used in the wave pattern.',
  'background.lineWavesColor2':         'Second colour used in the wave pattern.',
  'background.lineWavesColor3':         'Third colour used in the wave pattern.',
  'background.lineWavesOpacity':        'Overall opacity of the line waves layer.',
  'background.lineWavesBeatFreqStart':  'Lowest frequency that triggers the glow boost.',
  'background.lineWavesBeatFreqEnd':    'Highest frequency that triggers the glow boost.',
  'background.lineWavesBeatSensitivity':'Beat detection sensitivity. Lower = more sensitive.',
  'background.lineWavesBeatGlowBoost':  'How strongly a beat multiplies the wave brightness. 0 = no reaction.',
  // ── Radar ───────────────────────────────────────────────────────────────────────
  'background.radarEnabled':            'Animated radar sweep with concentric rings and spokes.',
  'background.radarBehindLogo':         'When set to "Behind logo", the effect is masked out at the logo position. "In front" lets it cover everything including the logo.',
  'background.radarBlendMode':          'How the radar layer blends with the layers beneath it.',
  'background.radarSpeed':              'Speed of the outward ring animation.',
  'background.radarScale':              'Zoom level of the radar. Lower = wider rings, higher = more compact.',
  'background.radarRingCount':          'Number of concentric rings visible at once.',
  'background.radarSpokeCount':         'Number of radial spokes emanating from the centre.',
  'background.radarRingThickness':      'Thickness of each ring line. Higher = thicker rings.',
  'background.radarSpokeThickness':     'Angular thickness of each spoke. Higher = wider spokes.',
  'background.radarSweepSpeed':         'Rotation speed of the sweep beam.',
  'background.radarSweepWidth':         'Width/sharpness of the sweep beam. Lower = wider beam.',
  'background.radarSweepLobes':         'Number of sweep lobes. 1 = single beam, higher = multiple beams.',
  'background.radarColor':              'Colour of the radar rings, spokes and sweep beam.',
  'background.radarBrightness':         'Overall brightness of the radar pattern.',
  'background.radarFalloff':            'How quickly the radar fades toward the edges.',
  'background.radarOpacity':            'Overall opacity of the radar layer.',
  'background.radarBeatFreqStart':      'Lowest frequency that triggers the glow boost.',
  'background.radarBeatFreqEnd':        'Highest frequency that triggers the glow boost.',
  'background.radarBeatSensitivity':    'Beat detection sensitivity. Lower = more sensitive.',
  'background.radarBeatGlowBoost':      'How strongly a beat multiplies the radar brightness. 0 = no reaction.',
  // ── Soft Aurora ───────────────────────────────────────────────────────────────
  'background.softAuroraEnabled':       'Smooth, flowing aurora borealis effect across the screen.',
  'background.softAuroraBehindLogo':    'When set to "Behind logo", the effect is masked out at the logo position. "In front" lets it cover everything including the logo.',
  'background.softAuroraBlendMode':     'How the aurora layer blends with the layers beneath it.',
  'background.softAuroraSpeed':         'Animation speed of the aurora flow.',
  'background.softAuroraScale':         'Spatial scale of the aurora noise. Higher = finer detail.',
  'background.softAuroraBrightness':    'Overall brightness of the aurora. Higher = more vivid.',
  'background.softAuroraColor1':        'Primary colour tint of the first aurora band.',
  'background.softAuroraColor2':        'Colour tint of the second aurora band.',
  'background.softAuroraNoiseFreq':     'Frequency of the Perlin noise driving the aurora shape.',
  'background.softAuroraNoiseAmp':      'Amplitude of the noise distortion. Higher = wilder bands.',
  'background.softAuroraBandHeight':    'Vertical position of the aurora band (0 = bottom, 1 = top).',
  'background.softAuroraBandSpread':    'How wide/spread out the aurora band is vertically.',
  'background.softAuroraOctaveDecay':   'How quickly higher noise octaves fade. Lower = more detail.',
  'background.softAuroraLayerOffset':   'Time offset between the two aurora layers for variation.',
  'background.softAuroraColorSpeed':    'Speed of the colour cycling within each aurora band.',
  'background.softAuroraOpacity':       'Overall opacity of the aurora layer.',
  'background.softAuroraBeatFreqStart': 'Lowest frequency that triggers the brightness boost.',
  'background.softAuroraBeatFreqEnd':   'Highest frequency that triggers the brightness boost.',
  'background.softAuroraBeatSensitivity':'Beat detection sensitivity. Lower = more sensitive.',
  'background.softAuroraBeatGlowBoost': 'How strongly a beat multiplies the aurora brightness. 0 = no reaction.',
