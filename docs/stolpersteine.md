...
- **🔴 ShaderMaterial-Uniforms IMMER via `mat.uniforms` updaten, NIE via `useMemo`-Objekt** (Session 21 / MagicRings): `useMemo(() => ({ uTime: { value: 0 } }), [])` erstellt ein Plain-Object. R3F/Three.js kann beim Zuweisen von `<shaderMaterial uniforms={obj}>` intern `Object.assign({}, {}, obj)` aufrufen — entstehendes Shallow-Copy hat andere Top-Level-Referenz. Mutationen am Original-Objekt (`uniforms.uTime.value = ...`) kommen dann nie beim Shader an. **Fix:** `matRef.current.uniforms.uTime.value = ...` — exakt wie `BackgroundPlane.tsx`. **Symptom:** Effekt ist sichtbar (initiale Werte gerendert), aber bewegt sich nicht und reagiert nicht auf Settings-Änderungen.
- **🔴 Full-Screen-Quad in orthografischer Kamera braucht dynamische Skalierung** (Session 21 / MagicRings): Die Kamera in `AudioScene.tsx` ist `orthographic` mit `zoom=1` → 1 world unit = 1 CSS pixel. Eine `PlaneGeometry(2, 2)` ist buchstäblich 2×2 Pixel groß — unsichtbar oder winzig. **Fix:** `mesh.scale.set(width, height, 1)` in `useFrame` (mit `width/height` aus `state.size`). `BackgroundPlane` nutzt zusätzlich Faktor 1.16 als Oversize-Margin für beat-scale. **Symptom:** Effekt erscheint nur als winziges Rechteck in der Mitte des Bildschirms.
