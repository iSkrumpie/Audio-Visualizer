...
└── lib/
    ...
    ├── settingsStore.ts     # zustand + persist(key='audiovisualizer:settings:v21', v=21)
...
- Key: `audiovisualizer:settings:v21`
- `version: 21`, `migrate: () => DEFAULT_SETTINGS` → bei Schema-Bump alles wipen
...
| `Strands` | `settings.background.strands*` | R3F-Komponente (seit Session 20 — war vorher ogl DOM-Overlay). PlaneGeometry(2,2) + ShaderMaterial, `mesh.scale.set(w,h,1)` in useFrame. `behindLogo` via `mesh.renderOrder` (4=vor Logo, 9=nach allem). AdditiveBlending. GLSL: alle lokalen Vars mit `str_` Prefix. **1× FreqBeatDetector** → Beat boostet uAmplitude +40% und uGlow +30%. |
| `LightRays` | `settings.background.lightRays*` | R3F-Komponente (seit Session 20 — war vorher ogl DOM-Overlay). God-Rays/crepuscular-rays-Shader. PlaneGeometry(2,2) + ShaderMaterial, AdditiveBlending. `uRayPos`/`uRayDir` aus origin-Setting berechnet. `behindLogo` via `mesh.renderOrder`. GLSL Prefix: `lr_`. **1× FreqBeatDetector** → Beat boostet uIntensityBoost, aktiviert uPulsating. |
| `LightPillar` | `settings.background.lightPillar*` | R3F-Komponente (seit Session 20 — war vorher standalone Three.js DOM-Overlay). Ray-Marching-Shader (3D Lichtsäule). PlaneGeometry(2,2) + ShaderMaterial mit `precision='highp'`, AdditiveBlending. `behindLogo` via `mesh.renderOrder`. GLSL Prefix: `lp_`. **1× FreqBeatDetector** → Beat skaliert uIntensity und uPillarWidth. |
| `FaultyTerminal` | `settings.background.faultyTerminal*` | R3F-Komponente. Retro-CRT-Terminal-Shader: prozedurales Digit-Grid, Scanlines, Glitch-Displacement, Barrel-Distortion. **1× FreqBeatDetector** → Beat-Glitch-Burst (uBeatGlitch) + Scanline-Flare. behindLogo via renderOrder. GLSL Prefix: `ft_`. Wichtig: `precision highp float;` (nicht mediump — Three.js WebGL2-Konflikt). |
...
-9.5 BackgroundFx (Particles+Rain+Snow) renderOrder=1
 -8  NebulaPlane
      Strands / LightRays / LightPillar  renderOrder=4 (behindLogo=true)
              ↑ renderOrder=4 = vor allen Logo-Layern (5-8). Das Logo-Mesh
                (transparent: true, Kreis-Clip) überdeckt den Effekt im Logo-Kreis.
...
-3.1 GPUParticles LineSegments (renderOrder=3)
 ...
 0.1  Fire-Ring RingGeometry ShaderMaterial (renderOrder=7)   ← zeichnet über Logo
      Strands / LightRays / LightPillar  renderOrder=9 (behindLogo=false) ← nach allem
      PostFX = stub, kein EffectComposer
      Outer-Glow Plane (renderOrder=5) sitzt hinter dem Logo, eigene z=-0.1 in useFrame

⚠️  `mesh.position.z` hat bei `depthTest:false`-Materialien KEINEN Effekt auf
    die Renderreihenfolge. Immer `mesh.renderOrder` setzen. Neue transparente
    Fullscreen-Effekte: `mesh.renderOrder = behindLogo ? 4 : 9` in useFrame.
```
...
3. TransportBar, ExportOverlay, etc. (HTML-Overlays)
```

**Hinweis (Session 20):** Strands, LightRays und LightPillar waren ursprünglich DOM-Overlays (Punkt 3 in der alten Liste). Sie wurden in Session 20 in den R3F SceneGraph portiert, weil DOM-Overlays im Export unsichtbar sind (Export erfasst nur `gl.domElement`). Die `behindLogo`-Funktionalität wird jetzt über `mesh.renderOrder` in `useFrame` gesteuert (kein CSS-Mask-Overhead mehr).
...
| light-rays | `LightRays.tsx` (`lightRaysBeatDetector`) | `settings.background.lightRaysBeatFreq*` + `lightRaysBeatSensitivity` | `rawFreqData`, `lightRaysBeatFreqStart`, `lightRaysBeatFreqEnd` |
| light-pillar | `LightPillar.tsx` (`lightPillarBeatDetector`) | `settings.background.lightPillarBeatFreq*` + `lightPillarBeatSensitivity` | `rawFreqData`, `lightPillarBeatFreqStart`, `lightPillarBeatFreqEnd` |
| faulty-terminal | `FaultyTerminal.tsx` (`faultyTerminalDetector`) | `settings.background.faultyTerminalBeatFreq*` + `faultyTerminalBeatSensitivity` | `rawFreqData`, `faultyTerminalBeatFreqStart`, `faultyTerminalBeatFreqEnd` |

**14 Instanzen gesamt** (1 global + 12 komponenten-spezifisch). Alle R3F-Komponenten, kein ogl-Overlay mehr. Alle nutzen `setSensitivity()` VOR `update()` im useFrame.
