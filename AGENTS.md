...
| `docs/stolpersteine.md` | §6 Bekannte Stolpersteine + §6.1 Export-Preview-Matching + §8 Build-Verifikation | Wenn du auf einen Bug stößt, den Export anfasst, oder `typecheck`/`build`-Probleme hast |
...
| `docs/session-log.md` | §10-17 alle Session-Notizen (Audio Detection, Hints, Fire v2, Sparks, Ring-UV, Inner Glow, UI Polish, Strands) | Wenn du verstehen willst warum etwas so implementiert ist wie es ist, oder nach früheren Bugfixes suchst |
